...
def main(args):
    ...
    #     # report = reports[0] # original line
    #     report = next((r for r in reports if r.strip()), '') #for experiments with k=1 (By Asaad) and checking ofr k=2 as well
    #     added = 1
    ...
    for doc in tqdm(range(len(ve_output))):
        ...
        # If the report starts with SEP, use the next non-empty report
        if reports[0].strip() == '':
            report = next((r for r in reports if r.strip()), '')
        else:
            # Otherwise, use the first report
            report = reports[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--m2trans_nli_model_path', default='F:\\Asaad_XREM_Repo\\ifcc\\resources\\model_medrad_19k')
    parser.add_argument('--input_path', default='', help = 'path to the output of the VE module') 
    parser.add_argument('--save_path', default='')
    parser.add_argument('--delimiter', default = '[SEP]')
    parser.add_argument('--topk', type = int, default = 2) # for ablation experiment E_RV_2
