...
class Predictor(BasePredictor):
    def setup(self):
        ...
        self.tokenizer = BertTokenizer.from_pretrained("emilyalsentzer/Bio_ClinicalBERT") #for bioclinicalbert

        bert_config_path = "configs/config_bert.json"
        self.model = VL_Transformer_ITM(
            text_encoder="emilyalsentzer/Bio_ClinicalBERT", config_bert=bert_config_path #for bioclinicalbert
        )
...
def pre_caption(caption, max_words=30):
    caption = (
        re.sub(
            r"([!?\"()*#;~])",   # removed punctuations: . , ' : so that these punctuations remain in actual text
            "",                      #.lower() removed as bioclinicalbert is cased
        )
        .replace("-", " ")
        .replace("/", " ")
    )
